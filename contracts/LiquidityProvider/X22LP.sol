...
contract X22LP is ReentrancyGuard {
    ...
    uint256 public liquidityProvidersAPY;
    ...
    event userRecieved(address user, uint256 amount, uint256 index);
    event feesTransfered(address user, uint256 amount, uint256 index);
    event loanTransfered(address recipient, uint256 amount, uint256 index);
    event loanRepayed(uint256 amount, uint256 index);
    event yieldAdded(uint256 amount, uint256 index);
    event walletNominated(address newOwner);
    event walletChanged(address oldOwner, address newOwner);
    event requestedWithdraw(address user, uint256 amount, uint256 index);
    ...
    function checkValidArray(uint256[N_COINS] memory amounts)
        internal
        pure
        returns (bool)
    {
    ...
    function _deposit(uint256[N_COINS] memory amounts) internal {
        ...
        for (uint8 i = 0; i < N_COINS; i++) {
            ...
            YieldPoolBalance = YieldPoolBalance.add(
                amounts[i].mul(10**18).div(10**decimal)
            );
        }
    }
        ...
        for (uint8 j = 0; j < amountSupplied[recipient].length; j++) {
            if (amountSupplied[recipient][j].remAmt > 0 && amount > 0) {
                if (amount >= amountSupplied[recipient][j].remAmt) {
                    ...
                    amountSupplied[recipient][j].remAmt = (
                        amountSupplied[recipient][j].remAmt
                    ).sub(amount);
                    amount = 0;
                }
    ...
    function _withdraw(uint256[N_COINS] memory amounts) internal {
        ...
        for (uint8 i = 0; i < N_COINS; i++) {
            decimal = tokens[i].decimals();
            YieldPoolBalance = YieldPoolBalance.sub(
                amounts[i].mul(10**18).div(10**decimal)
            );
        }
    }
    ...
    //1-USDC
    //2-USDT
    function calcXPTAmount(uint256 amount, uint256 _index)
        public
        view
        returns (uint256)
    {
    ...
    function calculateTotalToken(bool _supply) public view returns (uint256) {
        uint256 decimal;
        uint256 storedFeesTotal;
        for (uint8 i = 0; i < N_COINS; i++) {
            decimal = tokens[i].decimals();
            storedFeesTotal = storedFeesTotal.add(
                storedFees[i].mul(1e18).div(10**decimal)
            );
        }
        if (_supply) {
    ...
    function supply(uint256 amount, uint256 _index)
        external
        nonReentrant
        validAmount(amount)
    {
        ...
        uint256 balanceAmount = totalAmount.mul(poolPart).div(
            DENOMINATOR
        );
    ...
    function depositYield(uint256 amount, uint256 _index) external {
        uint256 decimal;
        decimal = tokens[_index].decimals();
        selfBalance = selfBalance.add(amount.mul(1e18).div(10**decimal));
        liquidityProvidersAPY = liquidityProvidersAPY.add(
            amount.mul(1e18).div(10**decimal)
        );
    ...
    function deposit() external onlyWallet {
        uint256[N_COINS] memory amounts;
        uint256 totalAmount;
        uint256 decimal;
        totalAmount = calculateTotalToken(false);
        uint256 balanceAmount = totalAmount.mul(poolPart).div(N_COINS).div(
            DENOMINATOR
        );
        uint256 tokenBalance;
        for (uint8 i = 0; i < N_COINS; i++) {
    ...
    function withdrawAll() external onlyWallet {
        ...
        for (uint8 i = 0; i < N_COINS; i++) {
            decimal = tokens[i].decimals();
            selfBalance = selfBalance.add(
                (tokens[i].balanceOf(address(this))).mul(1e18).div(10**decimal)
            );
        }
        YieldPoolBalance = 0;
    }
    ...
    function rebalance() external onlyWallet {
    ...
    function withdrawLoan(uint256[N_COINS] memory amounts, address _recipient)
        external
        onlyWallet
    {
        ...
        require(
            loanGiven.add(total) <=
                (calculateTotalToken(true).mul(loanPart).div(DENOMINATOR)),
            "Exceed limit"
        );
    ...
    function repayLoan(uint256[N_COINS] memory amounts) external {
        require(checkValidArray(amounts), "amount can't be zero");
        uint256 decimal;
        for (uint8 i = 0; i < N_COINS; i++) {
            if (amounts[i] > 0) {
                decimal = tokens[i].decimals();
                loanGiven = loanGiven.sub(
                    amounts[i].mul(1e18).div(10**decimal)
                );
                selfBalance = selfBalance.add(
                    amounts[i].mul(1e18).div(10**decimal)
                );
    ...
    function changeStrategy(address _strategy) external onlyWallet {
        require(YieldPoolBalance == 0, "Call withdrawAll function first");
        strategy = rStrategy(_strategy);
    }

    // for changing withdrawal fees
    function setWithdrawFees(uint128 _fees) external onlyWallet {
        require(_fees < 100, "Entered fees too high");
        fees = _fees;
    }

    function setCoolingPeriod(uint128 _period) external onlyWallet {
        coolingPeriod = _period; //in seconds
    }

    function changeLoanPart(uint256 _value) external onlyWallet {
        require(_value < DENOMINATOR, "Entered loanPart too high");
        loanPart = _value;
    }

    function getBalances(uint256 _index) public view returns (uint256) {
        return (
            tokens[_index].balanceOf(address(this)).sub(storedFees[_index])
        );
    }
}
